<script>
	...
	Rockerbox = {}
	Rockerbox.viewability = function(w,f,width,height) {

		this.outerFrame = w;
		this.topWindow = w.top;
		this.outerFrameMax = {
			width: 10000,
			height: 10000
		};
		...
		//this.getTopFrameWidthHeight()

		var domainVisible = "" !== w.document.domain;
		var isIE = -1 !== w.navigator.userAgent.toLowerCase().indexOf("msie");
		var F = "(()|(-moz-)|(-webkit-))";


		this.getElemStyle = function(elem,style,w) {
			var s = isIE ? elem.currentStyle[style] :
		...
		this.getTextAlignment = function(elem, value) {
	        var e = RegExp("^" + F + value + "$"),
				d = this.getElemStyle;
			return e.test(d(elem, "text-align", this.outerFrame)) || 
				e.test(d(elem, "textAlign", this.outerFrame)) || 
				e.test(d(elem, "align", this.outerFrame))
		}

		this.getScrollPosition = function(w) {
			w || (w = this.topWindow)
			var elem = w.document.documentElement,
				body = w.document.body,
				top  = scrollLeft = 0;

			elem && 0 != elem.clientWidth ? 	
				(top = w.pageYOffset || 
					body.scrollTop, scrollLeft = w.pageXOffset || 
					body.scrollLeft) : 
				(top = w.pageYOffset || 
					elem.scrollTop, scrollLeft = w.pageXOffset || 
					elem.scrollLeft);

			//console.log("here:", w.pageYOffset, body.scrollTop, top)
		...
		this.getTopFrameWidthHeight = function() {
			var f = this.outerFrame;
			while (f.parent != f) {
				this.outerFrameMax.width = Math.min(this.outerFrameMax.width,f.innerWidth);
				this.outerFrameMax.height = Math.min(this.outerFrameMax.height,f.innerHeight);
				f = f.parent
			}
			return this.outerFrameMax
		}

		this.getElemPosition = function(elem, width, height) {

			if (!elem) return { top: -1, left: -1, width: 0, height: 0 }
			...
			for (;i++, _elem;) {
				...
				if (_elem != elem && (_elem.offsetWidth < w)) {
				    w = _elem.offsetWidth;
				    if (_elem.offsetHeight < h) h = _elem.offsetHeight;
				}

				if (_elem.getBoundingClientRect) {
				    l = _elem.getBoundingClientRect();
				    p = this.getScrollPosition(f);

				    topPos += Math.round(l.top + p.top);
				    leftPos += Math.round(l.left + p.left);
				} else if (_elem.offsetParent) {
				    topPos += _elem.offsetTop; 
				    leftPos += _elem.offsetLeft; 
				    _elem = _elem.offsetParent;
				} else {
				    if (_elem.y) topPos += _elem.y
				 	if (_elem.x) leftPos += _elem.x; 
				    if (!isIE || i > 10) break;
				}

				_elem = f.frameElement;
				f = f.parent;
			}
			

			var offsetTop 	= elem.offsetTop,
				offsetLeft 	= elem.offsetLeft, // k
				offsetWidth = elem.offsetParent.offsetWidth;

			if (elem.parentElement != elem.offsetParent) {
				offsetTop 	= elem.offsetTop - elem.parentElement.offsetTop; 
				offsetLeft 	= elem.offsetLeft - elem.parentElement.offsetLeft;
				offsetWidth = elem.parentElement.offsetWidth;
			}

			offsetLeft = 
				this.getTextAlignment(elem,"center") || 
				this.getTextAlignment(elem.parentElement,"center") ?
					offsetLeft - Math.round((p - width) / 2) : 
					this.getTextAlignment(elem, "right") ?
						offsetLeft - (p - width) : 
						(width <= offsetLeft) ?
							width : 0;

			if (height <= offsetTop + 1) {
				var c = height; 
				var l = elem.offsetTop; 
				if ("top" != elem.style.verticalAlign) {
		...
		this.getPosition = function() {
			...
			var width = winHeight = 0,
				h = doc.compatMode ? doc.compatMode : "BackCompat";

			var scrollPos = this.getScrollPosition(w),
				topPos = scrollPos.top,
				leftPos = scrollPos.left;

			if ("BackCompat" === h && body){
				...
				if ("innerWidth" in elem && !isIE) {
					if (0 === width || w.innerWidth < width) width = w.innerWidth;
					if (0 === winHeight || w.innerHeight < winHeight) winHeight = w.innerHeight;
				}
			}
		...
		this.getVisPerc = function() {
			var elem = this.frame,
				...
				height = this.height,
				area = width*height,
				max = this.outerFrameMax;

			var elemPos = this.getElemPosition(elem,width,height),
				pagePos = this.getPosition(this.topWindow);

			var exposedArea = k = m = o = p = 0;

			elemPos.top <= pagePos.top ? 
				(k = elemPos, m = pagePos) : 
				(k = pagePos, m = elemPos);

			elemPos.left <= pagePos.left ? 
				(o = elemPos, p = pagePos) : 
				(o = pagePos, p = elemPos);

			var height = k.top  + k.height,
				width  = o.left + o.width;

			if (
				(k.top <= m.top) && 
				(m.top <= height && o.left <= p.left && p.left <= width) &&
				(1 < elemPos.width && 1 < elemPos.height && 1 < pagePos.height && 1 < pagePos.width)
			) {
				var pagePosLeft = pagePos.left + pagePos.width,
					elemPosLeft = elemPos.left + elemPos.width;

				var elemExposedX = Math.min(pagePosLeft, elemPosLeft) - Math.max(pagePos.left, elemPos.left),
					elemExposedY = Math.max(0,
						Math.min(
							(pagePos.top + pagePos.height) - (elemPos.top + elemPos.topOffset),
							elemPos.height
						)
					)

				exposedArea = Math.min(
					elemExposedX*elemExposedY,
					max.width*(max.height-elemPos.topOffset)
				)

			}
			var positionArea = (elemPos.height * elemPos.width)
			
	        return exposedArea / Math.max(positionArea,area);
  ...
  /*
	...
	xx = new Rockerbox.viewability(window,frame, w, h)
	xx.getVisPerc()
	setInterval(function(){
		console.log(xx.getVisPerc())
	},1000)
  */
