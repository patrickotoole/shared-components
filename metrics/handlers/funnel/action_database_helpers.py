...
class ActionDatabaseHelper(object):
    ...
    def _insert_into_tree(self, action, advertiser):
        ...
        if parent_node_id.empty:
            ...
            resp = self.requests.post('http://tree-synchronizer.marathon.mesos:31499/tree/visit_events_tree', data=ujson.dumps(data), headers={"Content-Type":"application/json"})
            parent_node_id = self.db.select_dataframe(GETPARENTNODE % advertiser_string)
        try:
        ...
        if node_does_not_exists:
            ...
            self.requests.post('http://tree-synchronizer.marathon.mesos:31499/tree/visit_events_tree', data=ujson.dumps(data),headers={"Content-Type":"application/json"})
    ...
    def _delete_from_tree(self, action_id):
        ...
        resp = self.requests.delete('http://tree-synchronizer.marathon.mesos:31499/tree/visit_events_tree?id={}'.format(action_id))
        logging.info(resp)
