...
RB.crusher.ui.action = (function(action) {
  ...
  action.show_similarity = function(wrapper) {

    var title = "Domain Similarity Clusters",
      series = ["clusters"],
      formatting = ".col-md-12.domain-clusters",
      description = "Domain similarity is calculated by a visit to multiple domains by a single user"
    ...
    var parentNode = wrapper.selectAll(".action-body").selectAll(".domain-clusters")
  ...
  action.show_clusters = function(wrapper) {
   ...
   action.show_similarity(wrapper)
