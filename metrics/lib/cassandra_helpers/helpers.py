...
class FutureHelpers:
    ...
    @staticmethod
    def run_next_update(result,iterable,run_future,always,success,categories,*args):
        """ 
        This is used to run the next future.
        """
        if type(result) is list:
            success['domains'](result,*args)
            success['categories'](categories,result,*args)
            success['domains_hour'](result,*args)
            success['category_hour'](categories,result,*args)
        else:
            pass
        ...
        try:
            ...
            start_time = time.time()
            def cb_with_result(result):
                FutureHelpers.run_next_update(result,iterable,run_future,always,success,categories,*args)
                always()
                #logging.info("%s %s" % (future._current_host.address,start_time-time.time()))
                return
    ...
    @staticmethod
    def future_queue_update(data,bound,callback,categories,queue_size=300,*args):
        """
        This will modify the args within the callback function and then return them
        after all the futures have responded.
        """
        ...
        for i in range(min(queue_size,length)):
            FutureHelpers.run_next_update(False,iterable,bound,step,callback,categories,*args)
