...
QUERY = "select distinct * from domain_idf where category_name is null and count > 5 and char_length(domain) < 29"
...
def write_to_sql_category(db,df):
    ...
    key = "domain"
    columns = df.columns
    loader.insert_df(df.reset_index(),"domain_category_only",["domain"],columns)
