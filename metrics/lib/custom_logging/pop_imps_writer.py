...
BASE_QUERY = "insert into pop_data (uid, url, domain, timestamp) values "
...
if __name__ == '__main__':
    ...
    def batch_insert(batch):
        base_string = "('%(uid)s', '%(url)s', '%(domain)s', '%(timestamp)s')"
    ...
    for message in consumer:
        if message is not None:
            ...
            if str(uid)[-4:] == '1123':
                timestamp = msg['bid_request']['timestamp']
                if url is not None and url != "" and url != " ":
                    batch.append({"uid":str(uid), "url":url.replace("'",""), "domain":domain.replace("'",""), "timestamp":timestamp})
                if len(batch)==100:
                    batch_insert(batch)
                    batch = []
