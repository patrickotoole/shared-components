...
def get_or_create_console():
    ...
    try:
        ...
        raise
    logging.debug("created a api console")
    CONSOLE = console
    return console
...
def get_report_id(request_url, request_json_form):
    ...
    logging.debug("Got report id: %s" % report_id)
    assert report_id
    return report_id
...
def get_report_url(report_id):
    ...
    if not url:
        ...
        logging.debug("report: %s status is %s" % (report_id, status))
        raise ValueError(status)
    logging.debug("report url is: %s" % url)
    return url
...
def get_report_resp(url):
    ...
    if not url.startswith('/'):
        url = '/' + url
    resp = _get_resp(url)
    logging.debug("successfully got response from url: %s" % url)
    return resp.text
