...
def runner(advertiser,pattern, func_name, base_url, cache_date="", indentifiers="test", connectors=False):
    ...
    script_name = "udf_{}".format(func_name)
    UR.accounting_entry_start(advertiser, pattern, "udf_{}".format(func_name), uuid_num)
    ...
    try:
        UR.insert(advertiser, pattern, "udf_{}".format(func_name), compress_data)
