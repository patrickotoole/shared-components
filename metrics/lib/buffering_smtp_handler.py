...
class BufferingSMTPHandler(logging.handlers.BufferingHandler):
    def __init__(self, fromaddr, toaddrs, subject, capacity, mailhost="smtp.sendgrid.net"):
    ...
    def flush(self):
        if len(self.buffer) > 0:
            try:
                ...
                smtp.login("ronjacobson", "rockerbox2013")
...
def test():
    FROM     = 'will@rockerbox.com'
    TO       = ['wmw71190@gmail.com']
    SUBJECT  = 'Test Logging email from Python logging module (buffering)'

    logger = logging.getLogger("")
    logger.setLevel(logging.DEBUG)
    logger.addHandler(BufferingSMTPHandler(FROM, TO, SUBJECT, 5000))
    for i in xrange(102):
