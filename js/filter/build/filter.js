(function (global, factory) {
	...
	typeof define === 'function' && define.amd ? define('filter', ['exports', 'd3'], factory) :
	(factory((global.filter = global.filter || {}),global.d3));
...
Filter.prototype = {
  draw: function () {
    ...
    var wrap = d3_updateable(this._target, ".filters-wrapper", "div", this.data(), function () {
      return 1;
    }).classed("filters-wrapper", true).style("padding-left", "10px").style("padding-right", "20px");

    var filters = d3_updateable(wrap, ".filters", "div", false, function (x) {
      return 1;
    }).classed("filters", true);

    var filter = d3_splat(filters, ".filter", "div", function (x) {
      return x;
    }, function (x, i) {
      return i + x.field;
  ...
  filterRow: function (_filter, fields, ops, value) {
    ...
    var remove = d3_updateable(_filter, ".remove", "a").classed("remove", true).style("float", "right").style("font-weight", "bold").html("&#10005;").on("click", function (x) {
    ...
    var filter = d3_updateable(_filter, ".inner", "div").classed("inner", true);

    var select = d3_updateable(filter, "select.field", "select", fields).classed("field", true).style("width", "165px").on("change", function (x, i) {
    ...
    d3_updateable(select, "option", "option").property("disabled", true).property("hidden", true).text("Filter...");

    d3_splat(select, "option", "option").text(function (x) {
      return x;
  ...
  drawOps: function (filter, ops, value) {
    ...
    var select = d3_updateable(filter, "select.op", "select", false, function (x) {
      return 1;
    ...
    var ops = d3_splat(select, "option", "option", new_ops, function (x) {
      return x.key;
    }).text(function (x) {
  ...
  drawInput: function (filter, value, op) {
    ...
    d3_updateable(filter, "input.value", "input").classed("value", true).style("padding-left", "10px").style("width", "150px").style("line-height", "1em").attr("value", value.value).on("keyup", function (x) {
      var t = this;
  ...
  filterFooter: function (wrap) {
    var footer = d3_updateable(wrap, ".filter-footer", "div", false, function (x) {
      return 1;
    }).classed("filter-footer", true).style("margin-bottom", "15px").style("margin-top", "10px");

    var self = this;

    d3_updateable(footer, ".add", "a", false, function (x) {
      return 1;
  ...
  _ops: {
    ...
    "contains": function (field, value) {
      return function (x) {
        var values = value.toLowerCase().split(",");
        return values.reduce(function (p, value) {
          return p + String(x[field]).toLowerCase().indexOf(String(value).toLowerCase()) > -1;
        }, 0) > 0;
      };
    }
  }
